...
public class ClienteDAO {
    ...
    public ResultSet consultarCliente(ClienteDTO clienteDTO, int opcao) {
        try {
            ...
            switch (opcao) {
                case 1:
                    comando = "SELECT c.* FROM cliente c WHERE nome_cli LIKE '"
                            + clienteDTO.getNome_cli() + "%' ORDER BY c.nome_cli";
                    break;
                case 2:
    ...
    public boolean alterarCliente(ClienteDTO clienteDTO) {
        try {
            ...
            System.out.println("Erro ao realizar ao atualizar o cliente na base de dados."
                    + "\nErro: " + e.getMessage());
